...
/** Digit count strictly before `index` in a formatted or raw string */
export function countDigitsBefore(str, index) {
    let count = 0;
...
/** Cursor index in formatted string after `digitsBefore` leading digits */
export function rupiahInputCursorAfterDigits(formatted, digitsBefore) {
    if (!formatted || digitsBefore <= 0) return 0;
    let count = 0;
